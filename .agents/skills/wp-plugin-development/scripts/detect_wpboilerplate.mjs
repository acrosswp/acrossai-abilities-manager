...
function main() {
	...
	const structurePresent = requiredFiles.every((f) =>
		statSafe(path.join(repoRoot, f))?.isFile()
	);
	...
	for (const phpPath of phpFiles) {
		...
		const hasBootstrapNs =
			...
			(detectedPrefix &&
				new RegExp(
					`^\\s*namespace\\s+${detectedPrefix.replace(/\\/g, '\\\\')}\\s*;`,
					'm'
				).test(txt));
		if (!hasBootstrapNs) {
			continue;
	...
	const buildArtifactsPresent =
		statSafe(path.join(repoRoot, 'build/js/backend.asset.php'))?.isFile() ??
		false;
	...
	const report = {
		...
		namespacePrefix:
			detectedPrefix ??
			(isWPBoilerplate ? 'WordPress_Plugin_Boilerplate' : null),
		bootstrapFile,
		header: pluginHeader,
		namespaceMap,
