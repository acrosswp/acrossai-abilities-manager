...
export function deleteSourceEntries(
	db: MemoryDatabase,
	sourcePath: string
): void {
	const ids = db
	...
	const deleteState = db.prepare(
		`DELETE FROM indexing_state WHERE source_path = ?`
	);
...
export function loadAllEntries(db: MemoryDatabase): MemoryEntryRecord[] {
	return db
		.prepare(
			`SELECT * FROM memory_entries ORDER BY source_path, line_start ASC, line_end ASC`
		)
...
export function loadIndexingState(db: MemoryDatabase): IndexingStateRecord[] {
	return db
		.prepare(`SELECT * FROM indexing_state`)
		.all() as IndexingStateRecord[];
}

export function loadIndexingStateMap(
	db: MemoryDatabase
): Map<string, IndexingStateRecord> {
...
export function loadDistinctSourcePaths(db: MemoryDatabase): string[] {
	const rows = db
		.prepare(`SELECT DISTINCT source_path FROM memory_entries`)
