...
export async function auditMemoryCache(
	...
	for (const row of state) {
		...
		if (!(await pathExists(fullPath))) {
			...
			issues.push({
				...
				recommendation:
					'Remove the stale cache rows and reindex the file if it returns.',
			});
			continue;
		}
		...
		if (fileHash !== row.hash) {
			...
			issues.push({
				...
				recommendation:
					'Run refresh-memory or rebuild-memory to resync the cache.',
			});
		}
	}
	...
	for (const entry of entries) {
		...
		if (!(await pathExists(fullPath))) {
			...
			issues.push({
				...
				recommendation:
					'Remove orphaned rows during refresh or rebuild.',
			});
		}
	}
	...
	for (const synthesisFile of synthesisFiles) {
		if (synthesisFile.words > config.retrieval.max_synthesis_words) {
			issues.push({
				...
				recommendation:
					'Trim synthesis output or raise the configured synthesis word limit.',
			});
		}
	}
	...
	for (const duplicate of duplicateKeys) {
		issues.push({
			...
			recommendation:
				'Review the duplicated source sections and consolidate if necessary.',
		});
	}
...
async function collectSynthesisFiles(
	...
	for (const feature of await fs.promises.readdir(specsRoot, {
		withFileTypes: true,
	})) {
