...
function fileCategory(entry: SearchResult): SectionCategory {
	...
	if (source.includes('project_context')) {
		return 'project-context';
	}
	if (combined.includes('security')) {
		return 'security';
	}
	if (combined.includes('accept') || combined.includes('deviation')) {
		return 'deviation';
	}
	if (source.includes('architecture')) {
		return 'architecture';
	}
	if (source.includes('decisions')) {
		return 'decision';
	}
	if (source.includes('bugs')) {
		return 'bug';
	}
	if (source.includes('worklog')) {
		return 'worklog';
	}
	return 'decision';
}
...
function summarizeBucket(
	...
	const filtered = entries.filter(
		(entry) => fileCategory(entry) === category
	);
...
export async function generateMemorySynthesis(
	...
	const synthesisPath = path.join(
		featureRoot,
		config.memory_synthesis_filename
	);
	...
	const featureMemory = (await pathExists(featureMemoryPath))
		...
		: '';
	const query =
		customQuery || [featureName, featureSpec, featureMemory].join('\n');
	let results = searchMemoryEntries(
		db,
		query,
		config,
		config.retrieval.max_memory_results
	);
	if (results.length === 0) {
		results = searchMemoryEntries(
			db,
			'',
			config,
			config.retrieval.max_memory_results
	...
	const items = results.map((entry, index) =>
		entryToSynthesisItem(
			entry,
			`${categoryPrefix(fileCategory(entry))}${index + 1}`
		)
	);
	...
	sections.push(
		formatItems(
			...
			limitLines(
				results
					.filter(
						(entry) => fileCategory(entry) === 'project-context'
					)
					.map((entry, index) =>
						bullet(
							`C${index + 1}`,
	...
	sections.push(
		summarizeBucket('Relevant Decisions', results, 'decision', 5, 'D')
	);
	sections.push(
		summarizeBucket(
	...
	sections.push(
		summarizeBucket('Accepted Deviations', results, 'deviation', 3, 'V')
	);
	sections.push(
		summarizeBucket(
	...
	sections.push(
		summarizeBucket('Related Historical Lessons', results, 'bug', 3, 'B')
	);
	sections.push(
		formatItems(
			'Conflict Warnings',
