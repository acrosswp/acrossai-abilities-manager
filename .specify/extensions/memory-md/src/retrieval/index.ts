...
export function scoreResult(result: SearchResult, query: string): SearchResult {
	...
	const headingScore =
		containsQueryTerm(result.section_heading ?? '', terms) * 0.25;
	...
	const score =
		fts + tagScore + headingScore + pathScore + contentScore + recency;

	const reason: string[] = [];
	if (tagScore > 0) {
		reason.push('tag match');
	}
	if (headingScore > 0) {
		reason.push('heading match');
	}
	if (pathScore > 0) {
		reason.push('path match');
	}
	if (contentScore > 0) {
		reason.push('content match');
	}
	if (recency > 0) {
		reason.push('recent');
	}
...
export function searchMemoryEntries(
	...
	const ftsCandidateLimit =
		config.retrieval.max_index_entries * FTS_OVER_FETCH_FACTOR;
	const normalizedQuery = buildFtsQuery(query);
	const candidates = searchFts(db, normalizedQuery, ftsCandidateLimit).map(
		(candidate) => scoreResult(candidate, query)
	);
	...
	return [...deduped.values()]
		.sort(
			(a, b) => b.score - a.score || (b.fts_rank ?? 0) - (a.fts_rank ?? 0)
		)
		.slice(0, limit);
}
...
export function entryToSynthesisItem(
	entry: SearchResult,
	label: string
): SynthesisSectionItem {
	return {
		...
		sectionHeading:
			entry.section_heading ?? path.basename(entry.source_path),
