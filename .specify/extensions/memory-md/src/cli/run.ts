...
async function runIndexMemory(
	projectRoot: string,
	refreshOnly = false
): Promise<void> {
	console.log(
		refreshOnly ? 'Refreshing memory index...' : 'Indexing memory files...'
	);
...
async function runSynthesize(
	...
	try {
		await ensureIndexedMemory(projectRoot, db, config);
		const synthesis = await writeMemorySynthesis(
			db,
			projectRoot,
			featurePath,
			config,
...
async function runAuditMemory(projectRoot: string): Promise<void> {
	...
	const { config, db } = createContext(projectRoot);
	try {
		const report = await auditMemoryCache(
			db,
			projectRoot,
			undefined,
			config
...
async function runRebuildMemory(projectRoot: string): Promise<void> {
	...
	try {
		...
		try {
			const result = await indexPhase1MemoryFiles(
				projectRoot,
				rebuiltDb,
				config,
				{
...
async function runTokenReport(
	...
	try {
		await ensureIndexedMemory(projectRoot, db, config);
		const { memoryRoot, specsRoot } = resolveProjectPaths(
			projectRoot,
			config
		);
		const featureRoot = path.resolve(projectRoot, featurePath);
		const featureSpecPath = path.join(featureRoot, 'spec.md');
		const featureMemoryPath = path.join(
			featureRoot,
			config.feature_memory_filename
		);
		...
		const memoryFiles = await discoverPhase1MemoryFiles(
			projectRoot,
			config
		);
		...
		const indexTokens = estimateTokens(
			baselineSources.find(
				(file) => path.basename(file.path) === 'INDEX.md'
			)?.content ?? ''
		);
		const selectedTokens =
			selectedTexts.reduce((sum, text) => sum + estimateTokens(text), 0) +
			indexTokens;
			...
			.filter(
				(file) =>
					!selectedResults.some(
						(entry) => file.file === entry.source_path
					)
			)
		...
		printHeader(
			`Token report for ${path.relative(projectRoot, featureRoot)}`
		);
		printKeyValue(
			'Baseline:',
			`${baselineTokens.toLocaleString()} estimated tokens`
		);
		printKeyValue(
		...
		printKeyValue(
			'Final synthesis:',
			`${synthesisTokens.toLocaleString()} estimated tokens`
		);
		if (baselineTokens > 0) {
			const reduction = (
				(1 - synthesisTokens / baselineTokens) *
				100
			).toFixed(1);
		...
		console.log(
			chalk.gray(
				'This is an estimated token count, not guaranteed provider billing usage.'
			)
		);
		console.log(chalk.bold('\nLargest avoided files:'));
		for (const file of avoidedFiles) {
			console.log(
				`- ${file.file} (${file.tokens.toLocaleString()} tokens)`
			);
		}
	} finally {
		closeDatabase(db);
...
export async function runCli(argv = process.argv): Promise<void> {
	...
	program
		...
		.requiredOption(
			'--feature <path>',
			'feature directory, for example specs/001-auth'
		)
		.option(
			'--query <text>',
			'optional search query to override automatic feature query'
		)
	...
	program
		...
		.description(
			'Compare estimated token usage between baseline and optimized flows'
		)
	...
	program
		.command('doctor')
		.description(
			'[planned] Validate environment, config, and optimizer prerequisites'
		)
		.action(() => {
			console.log(
	...
	program
		...
		.requiredOption(
			'--file <relpath>',
			'relative path to detail file (e.g., ARCHITECTURE.md)'
		)
		.option(
			'--status <type>',
			'active, deprecated, or superseded',
			'active'
		)
		.option(
			'--content <markdown>',
