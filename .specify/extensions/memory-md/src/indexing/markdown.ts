...
function splitLongText(
	rawContent: string,
	maxWords = SECTION_WORD_TARGET_MAX
): string[] {
...
function splitIntoSentenceBlocks(
	rawContent: string,
	maxWords: number
): string[] {
...
export function parseMarkdownFile(
	...
	for (let index = 0; index < lines.length; index += 1) {
		...
		if (headingMatch) {
			...
			currentHeadingPath = [
				fileStem,
				...headingStack.map((frame) => frame.title),
			];
			currentLines = [];
...
function headingWords(pathParts: string[]): string[] {
	return pathParts.flatMap((part) =>
		part.split(/[^A-Za-z0-9]+/g).filter(Boolean)
	);
}
