...
function reducer(state = DEFAULT_STATE, action) {
	switch (action.type) {
		...
		case SET_MCP_SERVERS:
			return {
				...state,
				mcpServers: Array.isArray(action.servers) ? action.servers : [],
			};
		...
		case UPDATE_ABILITY: {
			const updated = state.abilities.map((ability) =>
				ability.slug === action.ability.slug
...
const actions = {
	...
	toggleAllowed:
		...
		async ({ dispatch, select }) => {
			...
			dispatch({
				type: UPDATE_ABILITY,
				ability: { slug, site_allowed: siteAllowed },
			});
			...
			try {
				...
				dispatch({
					...
					ability: {
						slug: result.slug,
						site_allowed: result.site_allowed,
		...
		async ({ dispatch, select }) => {
			...
			dispatch({
				...
				ability: {
					...
					has_override: false,
					site_allowed: prev
						? (prev._registry?.site_allowed ?? null)
						: null,
					readonly: prev ? (prev._registry?.readonly ?? null) : null,
					destructive: prev
						? (prev._registry?.destructive ?? null)
						: null,
					idempotent: prev
						? (prev._registry?.idempotent ?? null)
						: null,
					show_in_rest: prev
						? (prev._registry?.show_in_rest ?? null)
						: null,
					show_in_mcp: prev
						? (prev._registry?.show_in_mcp ?? null)
						: null,
					mcp_type: prev ? (prev._registry?.mcp_type ?? null) : null,
					mcp_servers: prev
						? (prev._registry?.mcp_servers ?? null)
						: null,
