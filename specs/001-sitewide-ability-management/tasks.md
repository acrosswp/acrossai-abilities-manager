...
- [ ] T002 [P] Update `webpack.config.js` to spread `defaultConfig.entry` and add entries `'js/sitewide' → './src/js/sitewide/index.js'` and `'css/sitewide' → './src/scss/sitewide/admin.scss'` — key format MUST match existing project convention (`'js/frontend'`, `'css/frontend'`, etc.; see `webpack.config.js` lines 65–76)
