...
- [ ] T014 Create `includes/Modules/Sitewide/index.php` (one-line directory sentinel: `<?php // Silence is golden.`) and create `includes/Modules/Sitewide/AcrossAI_Sitewide_Module.php` — class `AcrossAI_Sitewide_Module` extends `AcrossAI_Module_Base`; `get_name(): string` returns `'sitewide'`; `register_hooks( $loader ): void` calls `$loader->add_action( 'rest_api_init', $controller, 'register_routes' )` where `$controller = new AcrossAI_Sitewide_Rest_Controller()`
- [ ] T015 Create `includes/Modules/Sitewide/AcrossAI_Sitewide_Rest_Controller.php` skeleton — constant `REST_NAMESPACE = 'acrossai-abilities-manager/v1'`; `register_routes()` stub (to be populated per user story phase); `check_permission(): bool|\WP_Error` — returns `WP_Error( 'rest_forbidden', ..., [ 'status' => 403 ] )` when `current_user_can( 'manage_options' )` fails OR nonce invalid via `wp_verify_nonce( $request->get_header('X-WP-Nonce'), 'wp_rest' )`; returns `true` on success; PHPDoc `@return bool|\WP_Error`; MUST NOT be typed as `: bool` alone — PHPStan L8 requires the union type since the method returns both `WP_Error` objects and `true` (RF-01 fix)
...
- [ ] T025 [US2] Add `toggle_ability()` handler and `POST /sitewide/abilities/{slug}/toggle` route to `includes/Modules/Sitewide/AcrossAI_Sitewide_Rest_Controller.php` — route arg: `slug` (string, required, URL-encoded); body arg: `site_allowed` (bool, required, sanitized via `AcrossAI_Sanitizer::sanitize_tri_state()`); `toggle_ability()` validates ability exists via `wp_get_ability( $slug )`; calls `AcrossAI_Sitewide_Query::save_override()` with `site_allowed` field only; returns `{ slug, site_allowed, has_override }` per rest-api.md contract; fires `acrossai_abilities_sitewide_after_save` action
...
- [ ] T028 [US3] Add `get_ability()`, `save_override()`, and `get_mcp_servers()` handlers and their routes to `includes/Modules/Sitewide/AcrossAI_Sitewide_Rest_Controller.php` — `GET /sitewide/abilities/{slug}`: validate slug, merge registry + override, return Effective Ability shape; `POST /sitewide/abilities/{slug}`: sanitize each field via `AcrossAI_Sanitizer`, call `AcrossAI_Ability_Merger::is_all_default()`, return `unchanged: true` if no diff (FR-024/025), else call `AcrossAI_Sitewide_Query::save_override()` (sets `show_in_mcp = NULL` and `mcp_servers = NULL` when "Keep as Default" — i.e., both submitted as null — per FR-017), fires `acrossai_abilities_sitewide_before_save` and `acrossai_abilities_sitewide_after_save`; `GET /sitewide/mcp-servers`: guard with `class_exists( 'WP\\MCP\\Core\\McpAdapter' )`, return `McpAdapter::instance()->get_servers()` as `[{ id, label }]` array or `[]` when adapter absent
...
- [ ] T034 [US4] Add `delete_override()` handler and `DELETE /sitewide/abilities/{slug}` route to `includes/Modules/Sitewide/AcrossAI_Sitewide_Rest_Controller.php` — validate slug exists in registry; call `AcrossAI_Sitewide_Query::delete_override_by_slug()`; return `{ slug, deleted: true }` if a record was deleted or `{ slug, deleted: false, message: '...' }` if no override existed (per rest-api.md contract)
...
- T005, T006, and T006b can run in parallel (different files, no dependencies)
